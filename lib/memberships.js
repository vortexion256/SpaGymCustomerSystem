...
export async function enrollClient(enrollmentData, currentUser = null) {
  try {
    ...
    // Filter for memberships that haven't expired yet
    const now = new Date();
    const activeEnrollments = activeEnrollmentsSnapshot.docs.filter(doc => {
      const data = doc.data();
      return data.expiryDate?.toDate() > now;
    });

    if (activeEnrollments.length > 0) {
      return { 
        success: false, 
        error: 'Client is already enrolled in an active membership. Please cancel or wait for the current membership to expire before enrolling in a new one.' 
      };
    }
