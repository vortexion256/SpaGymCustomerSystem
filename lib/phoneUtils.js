...
export function isValidPhoneFormat(phoneNumber) {
  ...
  // - 12 digits starting with 256 (e.g., 256782830524) - will be normalized
  // - 13 digits starting with +256 (e.g., +256782830524) - will be normalized

  return (
    /^0\d{9}$/.test(cleaned) || // 10 digits starting with 0
    /^[789]\d{8}$/.test(cleaned) || // 9 digits starting with 7-9
    /^256[789]\d{8}$/.test(cleaned) || // 12 digits with country code
    /^\+?256[789]\d{8}$/.test(cleaned.replace(/\+/g, '')) // 13 digits with +256
  );
}
...
export function normalizeSinglePhoneNumber(phoneNumber) {
  ...
  // If number is 12 digits and starts with country code (e.g., 256 for Uganda)
  // Extract the last 9 digits and add leading 0
  // Example: "256782830524" -> "0782830524"
  if (/^256[789]\d{8}$/.test(cleaned)) {
    return { normalized: '0' + cleaned.substring(3), isValid: true, original };
  }

  // If number is 13 digits and starts with +256, extract and normalize
  const cleanedPhone = cleaned.replace(/\+/g, '');
  if (/^256[789]\d{8}$/.test(cleanedPhone)) {
